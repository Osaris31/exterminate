...
public final class PingPongBuffer {
	 ...
	 * @param fsaa */
	public PingPongBuffer (int width, int height, Format frameBufferFormat, boolean hasDepth, boolean fsaa) {
		ownResources = true;
		

			owned1 = new FrameBuffer(frameBufferFormat, width, height, hasDepth);
			if(fsaa) {
				owned2 = new FrameBuffer(frameBufferFormat, (int)(width*ExterminateGame.fsaa), (int)(height*ExterminateGame.fsaa), hasDepth, false);
			}
			else {
				owned2 = new FrameBuffer(frameBufferFormat, (int)(width), (int)(height), hasDepth, false);
			}
		
		/*
		EXTFramebufferMultisample.glRenderbufferStorageMultisampleEXT(owned1.getColorBufferTexture().glTarget, 4, GL20.GL_RGBA, width, height);
		EXTFramebufferMultisample.glRenderbufferStorageMultisampleEXT(owned2.getColorBufferTexture().glTarget, 4,  GL20.GL_RGBA, width, height);
		*/
		set(owned1, owned2);
	}
