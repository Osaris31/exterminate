...
public final class VertexAttributes implements Iterable<VertexAttribute>, Comparable<VertexAttributes> {
	...
	public static final class Usage {
		...
		public static final int NormalPacked = 2048;
		public static final int TextureCoordinatesPacked = 4096;
	}
	...
	private int calculateOffsets () {
		...
		for (int i = 0; i < attributes.length; i++) {
			...
			if (attribute.usage == VertexAttributes.Usage.ColorPacked)
				count += 4;
			else if (attribute.usage == VertexAttributes.Usage.NormalPacked)
				count += 4; //OSARIS c'est là que t'as bossé.
			else if (attribute.usage == VertexAttributes.Usage.TextureCoordinatesPacked)
				count += 4; //OSARIS c'est là que t'as bossé.
			else
