...
public abstract class GLFrameBuffer<T extends GLTexture> implements Disposable {
	...
	private void build () {
		...
		if (hasDepth) {
			...
			if(ExterminateGame.useGL3) gl.glRenderbufferStorage(GL20.GL_RENDERBUFFER, GL30.GL_DEPTH24_STENCIL8, colorTexture.getWidth(), colorTexture.getHeight());
			else 			gl.glRenderbufferStorage(GL20.GL_RENDERBUFFER, GL20.GL_DEPTH_COMPONENT16, colorTexture.getWidth(),
		...
		if (hasDepth) {
			if(ExterminateGame.useGL3) gl.glFramebufferRenderbuffer(GL20.GL_FRAMEBUFFER, GL30.GL_DEPTH_STENCIL_ATTACHMENT, GL20.GL_RENDERBUFFER, depthbufferHandle);
			else gl.glFramebufferRenderbuffer(GL20.GL_FRAMEBUFFER, GL20.GL_DEPTH_ATTACHMENT, GL20.GL_RENDERBUFFER, depthbufferHandle);
		}
