...
/**
 * OSARIS : modif pour utiliser shader de polices à chaque draw.
 */
	...
	public void draw (Batch spriteBatch) {
		...
		for (int j = 0, n = pageVertices.length; j < n; j++) {
			if (idx[j] > 0) { // ignore if this texture has no glyphs
				float[] vertices = pageVertices[j];
				if(InGame.batcherFont!=null && InGame.batcherFont.drawing) {
					InGame.batcherFont.draw(regions.get(j).getTexture(), vertices, 0, idx[j]); //BY OSARIS					
				}
				else {
					spriteBatch.draw(regions.get(j).getTexture(), vertices, 0, idx[j]); //BY OSARIS
				}
			}
		}
	}
