...
public class ParticleController implements Json.Serializable, ResourceData.Configurable{
	...
	public void rotate(Vector3 axis, float angle){
		...
		if(axis.y>0.99) {
			DynamicsInfluencer di = this.findInfluencer(DynamicsInfluencer.class);
	        DynamicsModifier dm;
	        for (int j = 0; j < di.velocities.size; j++) {
	            ...
	            if(dm instanceof PolarAcceleration) {
	                ((PolarAcceleration) dm).phiValue.setHigh(90-20, 90+20);

	                ((PolarAcceleration) dm).thetaValue.setHigh(90-angle-20, 90-angle+20);
	...
	public void update(){
		deltaTime = MyParticules.deltaTimeS;
		deltaTimeSqr = MyParticules.deltaTimeSqrS;
